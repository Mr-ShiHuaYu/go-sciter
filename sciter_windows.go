...
//
// --- PE Import Table Fix (CRITICAL for CGO stability) ----------------------
...
// Eliminates half-initialised TLS / singleton state that could cause crashes
...
// branch only).  Using C rather than Go for window creation avoids VTable-slot
// offset mistakes and __stdcall parameter ordering bugs that plagued the Go
// syscall-based attempts.
...
func CreateWindow(createFlags WindowCreationFlag, rect *Rect, delegate uintptr, delegateParam uintptr, parent C.HWINDOW) C.HWINDOW {
	// =========================================================================
	// CGO HELPER — DELEGATE TO PURE C TO AVOID VTABLE ISSUES
	// =========================================================================
	...
	// that invokes SciterCreateWindow via the vtable with correct C calling
	// conventions.
	// =========================================================================
