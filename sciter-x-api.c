...
/* --- CGO FIX: try PE Import Table (static import) FIRST, fallback to
 ...
 * eliminates the multi-threaded DllMain/TLS race that could cause crashes
...
ISciterAPI *_SAPI(ISciterAPI *ext) {
  ...
  if (!_api) {
    HMODULE hm = NULL;
    /* ---- PATH A: PE Import Table (static import) — the CGO fix! ---- */
  ...
  assert(_api);
  if (!_api) {
    MessageBox(NULL, TEXT("Sciter engine not found, quiting"),
               TEXT("Error"), MB_OK);
    exit(-1);
  }
  return _api;
 ...
 *
 * CGO helper: calls SciterCreateWindow via C rather than Go syscall to avoid
 * VTable slot offset mistakes and __stdcall parameter ordering bugs that
 * plagued the Go syscall-based attempts.
 *
 * Go side calls this via CGO as:
 *   HWINDOW C.XPBootstrapAndCreateWindow(UINT flags, RECT* frame, HWINDOW parent)
 * ===========================================================================
 */
#if defined(WINDOWS)
#ifndef BAD_HWINDOW
 ...
 * declaration the call would fall through to implicit C int-return and cause
 * a pointer/truncation + conflict with the real SAPI() later. */
...
HWINDOW XPBootstrapAndCreateWindow(UINT creationFlags,
  ...
  /* SciterCreateWindow — VTable signature expects LPRECT (non-const RECT*)
   * so cast away const.  In Sciter API v10, the 3rd and 4th parameters
   * (delegate/delegateParam) are reserved and MUST be NULL. */
  HWINDOW hwnd = api->SciterCreateWindow(creationFlags, (LPRECT)prc, NULL, NULL, parent);
  if (hwnd == NULL) {
    return BAD_HWINDOW;
  }
  return hwnd;
